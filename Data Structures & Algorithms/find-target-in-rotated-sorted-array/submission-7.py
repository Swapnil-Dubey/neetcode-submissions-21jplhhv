...
#           -1000 <= target <= 1000
        ...
        while l<=r:
            ...
            if nums[l]<=nums[mid]:
                if nums[l]<=target<=nums[mid]: # just remember that for this problem, u have to split cases into which half is sorted
                #                           then search within the sorted half (if target is in the sorted range then search there)
                #                           otherwise search in the unsorted(pivot) half
                    r = mid-1
                else:
                    l = mid+1
